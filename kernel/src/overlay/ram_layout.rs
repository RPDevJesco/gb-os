...
//! Supports Pokemon Yellow (Gen 1) and Pokemon Crystal (Gen 2).
//!
//! References:
...
impl Game {
    ...
    ///
    /// Supports:
    /// - Gen 1: Red, Blue, Yellow (all use same memory layout)
    /// - Gen 2: Gold, Silver, Crystal (all use same memory layout)
    pub fn detect(rom_name: &str) -> Self {
        // Case-insensitive check without allocation
        let bytes = rom_name.as_bytes();

        // Gen 1 games (Red, Blue, Yellow) - all share Yellow's layout
        if Self::contains_ignore_case(bytes, b"RED") ||
            Self::contains_ignore_case(bytes, b"BLUE") ||
            Self::contains_ignore_case(bytes, b"YELLOW") {
            return Game::Yellow;  // Yellow layout works for all Gen 1
        }

        // Gen 2 games (Gold, Silver, Crystal) - all share Crystal's layout
        if Self::contains_ignore_case(bytes, b"GOLD") ||
            Self::contains_ignore_case(bytes, b"SILVER") ||
            Self::contains_ignore_case(bytes, b"CRYSTAL") {
            return Game::Crystal;  // Crystal layout works for all Gen 2
        }
...
pub mod yellow {
    ...
    pub const ENEMY_MON_SPECIES: u16 = 0xCFE5;  // 1 byte
    ...
    /// 0x50 = terminator, 0x7F = space, 0x80-0x99 = A-Z (uppercase)
...
pub mod crystal {
    ...
    // --- Play Time ---
    pub const PLAY_TIME_HOURS: u16 = 0xD4B7;    // 2 bytes
    pub const PLAY_TIME_MINUTES: u16 = 0xD4B9;  // 1 byte
    pub const PLAY_TIME_SECONDS: u16 = 0xD4BA;  // 1 byte
    ...
    pub const MON_UNUSED: u16 = 33;             // 1 byte
...
impl<'a> RamReader<'a> {
    ...
    /// Read 3-byte BCD value as integer (for money)
    ...
    pub fn read_party_pokemon(&self, slot: u8) -> Option<Pokemon> {
        ...
        match self.game {
            Game::Yellow => self.read_pokemon_gen1(slot),
            Game::Crystal => self.read_pokemon_gen2(slot),
            Game::Unknown => None,
        }
    }

    fn read_pokemon_gen1(&self, slot: u8) -> Option<Pokemon> {
    ...
    fn read_pokemon_gen2(&self, slot: u8) -> Option<Pokemon> {
    ...
    pub fn read_trainer(&self) -> TrainerData {
        match self.game {
            Game::Yellow => self.read_trainer_gen1(),
            Game::Crystal => self.read_trainer_gen2(),
    ...
    fn read_trainer_gen1(&self) -> TrainerData {
        let mut name = [0u8; 11];
        for i in 0..11 {
    ...
    fn read_trainer_gen2(&self) -> TrainerData {
        let mut name = [0u8; 11];
        for i in 0..11 {
        ...
        TrainerData {
            name,
            money: self.read_bcd24(crystal::MONEY),
    ...
    /// Get enemy Pokemon info during battle
    pub fn read_enemy_pokemon(&self) -> Option<(u8, u16, u8)> {
        ...
        let (species_addr, hp_addr, level_addr) = match self.game {
            Game::Yellow => (yellow::ENEMY_MON_SPECIES, yellow::ENEMY_MON_HP, yellow::ENEMY_MON_LEVEL),
            Game::Crystal => (crystal::ENEMY_MON_SPECIES, crystal::ENEMY_MON_HP, crystal::ENEMY_MON_LEVEL),
