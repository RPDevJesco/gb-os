...
/// Platform trait that ties everything together.
pub trait Platform {
    /// Serial type for this platform.
    type Serial: Serial;

    /// GPIO type for this platform.
    type Gpio: Gpio;

    /// Timer type for this platform.
    type Timer: Timer;
    ...
    /// Called very early, should set up clocks, basic I/O.
    fn init() -> Result<Self>
    where
        Self: Sized;

    /// Get reference to serial interface.
    fn serial(&mut self) -> &mut Self::Serial;

    /// Get reference to GPIO interface.
    fn gpio(&mut self) -> &mut Self::Gpio;

    /// Get reference to timer interface.
    fn timer(&self) -> &Self::Timer;

    /// Halt the system (used on fatal errors).
    fn halt() -> ! {
        loop {
            // Platform-specific halt instruction should be used
